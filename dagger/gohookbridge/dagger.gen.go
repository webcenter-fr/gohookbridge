...
func (r Gohookbridge) MarshalJSON() ([]byte, error) {
	var concrete struct{}
	return json.Marshal(&concrete)
}

func (r *Gohookbridge) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Gohookbridge":
		...
		case "Ci":
			...
			if inputArgs["repositoryName"] != nil {
				err = json.Unmarshal([]byte(inputArgs["repositoryName"]), &repositoryName)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg repositoryName", err))
				}
			}
			...
			var pushLatest bool
			if inputArgs["pushLatest"] != nil {
				err = json.Unmarshal([]byte(inputArgs["pushLatest"]), &pushLatest)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg pushLatest", err))
				}
			}
			var skipPush bool
			if inputArgs["skipPush"] != nil {
				err = json.Unmarshal([]byte(inputArgs["skipPush"]), &skipPush)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg skipPush", err))
				}
			}
			var skipK8S bool
			if inputArgs["skipK8s"] != nil {
				err = json.Unmarshal([]byte(inputArgs["skipK8s"]), &skipK8S)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg skipK8s", err))
				}
			}
			var dryRun bool
			...
			return (*Gohookbridge).Ci(&parent, ctx, source, version, registry, repositoryName, registryUsername, registryPassword, pushLatest, skipPush, skipK8S, dryRun, channelId, timeout)
		default:
