package service

import (
	"context"
	"testing"

	"github.com/webcenter-fr/gohookbridge/internal/domain"
	"gotest.tools/v3/assert"
)

func newTokenService(t *testing.T) *Service {
	t.Helper()
	fake := newFakeRepository()
	svc := NewService(fake, nil)
	return svc
}

func TestGenerateAccessToken(t *testing.T) {
	raw, hash, err := GenerateAccessToken()
	assert.NilError(t, err)
	assert.Assert(t, len(raw) > 0)
	assert.Assert(t, len(hash) > 0)
	assert.Assert(t, raw != hash, "raw and hash should be different")
	assert.Equal(t, len(raw), 64, "raw token should be 64 hex chars")
	assert.Equal(t, len(hash), 64, "hash should be 64 hex chars")

	// Verify the hash matches
	assert.Equal(t, hash, HashToken(raw))
}

func TestHashToken_Deterministic(t *testing.T) {
	h1 := HashToken("test-token-value")
	h2 := HashToken("test-token-value")
	assert.Equal(t, h1, h2)

	h3 := HashToken("different-token")
	assert.Assert(t, h1 != h3)
}

func TestCreateAccessToken(t *testing.T) {
	svc := newTokenService(t)
	ctx := context.Background()

	assert.NilError(t, svc.CreateChannel(ctx, &domain.Channel{ID: "test-chan"}))

	raw, token, err := svc.CreateAccessToken(ctx, "test-chan", "my-token", "produce")
	assert.NilError(t, err)
	assert.Assert(t, len(raw) > 0)
	assert.Equal(t, token.Name, "my-token")
	assert.Equal(t, token.Scope, "produce")
	assert.Equal(t, token.TokenHash, HashToken(raw))

	ch, err := svc.GetChannel(ctx, "test-chan")
	assert.NilError(t, err)
	assert.Equal(t, ch.AccessMode, "token")
	assert.Equal(t, len(ch.AccessTokens), 1)
	assert.Equal(t, ch.AccessTokens[0].Name, "my-token")

	t.Run("invalid scope defaults to both", func(t *testing.T) {
		ch2 := &domain.Channel{ID: "test-chan2"}
		assert.NilError(t, svc.CreateChannel(ctx, ch2))
		_, token2, err := svc.CreateAccessToken(ctx, "test-chan2", "bad-scope", "invalid")
		assert.NilError(t, err)
		assert.Equal(t, token2.Scope, "both")
	})

	t.Run("channel not found", func(t *testing.T) {
		_, _, err := svc.CreateAccessToken(ctx, "nonexistent", "test", "both")
		assert.Assert(t, err != nil)
	})
}

func TestDeleteAccessToken(t *testing.T) {
	svc := newTokenService(t)
	ctx := context.Background()

	assert.NilError(t, svc.CreateChannel(ctx, &domain.Channel{ID: "test-chan"}))

	_, token1, err := svc.CreateAccessToken(ctx, "test-chan", "token-1", "produce")
	assert.NilError(t, err)
	_, token2, err := svc.CreateAccessToken(ctx, "test-chan", "token-2", "consume")
	assert.NilError(t, err)

	assert.NilError(t, svc.DeleteAccessToken(ctx, "test-chan", token1.ID))

	ch, err := svc.GetChannel(ctx, "test-chan")
	assert.NilError(t, err)
	assert.Equal(t, len(ch.AccessTokens), 1)
	assert.Equal(t, ch.AccessTokens[0].Name, "token-2")

	t.Run("does not change access mode when last token is deleted", func(t *testing.T) {
		assert.NilError(t, svc.DeleteAccessToken(ctx, "test-chan", token2.ID))
		ch, err := svc.GetChannel(ctx, "test-chan")
		assert.NilError(t, err)
		assert.Equal(t, len(ch.AccessTokens), 0)
		assert.Equal(t, ch.AccessMode, "token")
	})

	t.Run("channel not found returns error", func(t *testing.T) {
		err := svc.DeleteAccessToken(ctx, "nonexistent", "some-id")
		assert.Assert(t, err != nil)
	})
}

func TestValidateChannelToken(t *testing.T) {
	svc := newTokenService(t)
	ctx := context.Background()

	assert.NilError(t, svc.CreateChannel(ctx, &domain.Channel{ID: "test-chan"}))

	produceRaw, _, err := svc.CreateAccessToken(ctx, "test-chan", "produce-token", "produce")
	assert.NilError(t, err)

	consumeRaw, _, err := svc.CreateAccessToken(ctx, "test-chan", "consume-token", "consume")
	assert.NilError(t, err)

	bothRaw, _, err := svc.CreateAccessToken(ctx, "test-chan", "both-token", "both")
	assert.NilError(t, err)

	t.Run("valid produce token for produce scope", func(t *testing.T) {
		assert.Assert(t, svc.ValidateChannelToken(ctx, "test-chan", produceRaw, "produce"))
	})

	t.Run("produce token rejected for consume scope", func(t *testing.T) {
		assert.Assert(t, !svc.ValidateChannelToken(ctx, "test-chan", produceRaw, "consume"))
	})

	t.Run("valid consume token for consume scope", func(t *testing.T) {
		assert.Assert(t, svc.ValidateChannelToken(ctx, "test-chan", consumeRaw, "consume"))
	})

	t.Run("consume token rejected for produce scope", func(t *testing.T) {
		assert.Assert(t, !svc.ValidateChannelToken(ctx, "test-chan", consumeRaw, "produce"))
	})

	t.Run("both scope token valid for produce", func(t *testing.T) {
		assert.Assert(t, svc.ValidateChannelToken(ctx, "test-chan", bothRaw, "produce"))
	})

	t.Run("both scope token valid for consume", func(t *testing.T) {
		assert.Assert(t, svc.ValidateChannelToken(ctx, "test-chan", bothRaw, "consume"))
	})

	t.Run("invalid token rejected", func(t *testing.T) {
		assert.Assert(t, !svc.ValidateChannelToken(ctx, "test-chan", "invalid-token-value", "produce"))
	})

	t.Run("nonexistent channel returns false", func(t *testing.T) {
		assert.Assert(t, !svc.ValidateChannelToken(ctx, "nonexistent", bothRaw, "produce"))
	})
}
