...
│   │   └── static/             # Nuxt static output (gitignored, regenerated by `make build`)
│   ├── nats/                   # Embedded NATS broker + ring buffer
│   ├── store/                  # Raft + BoltDB persistence layer
│   │   ├── raft.go             # Raft cluster setup, membership, clean state
│   │   ├── raft_discovery.go   # PeerResolver: static/DNS/single-node discovery
│   │   ├── raft_streamlayer.go # Plaintext/TLS stream layers + DNS re-resolve
│   │   ├── raft_tls.go         # Raft mTLS: embedded CA + K8s Secret sharing
...
1. `cd web && npm ci && npm run build` — runs `nuxt generate` and copies `.output/public` to `gohookbridge/web/static/`
2. `go build -o bin/gohookbridge ./cmd/gohookbridge` — embeds the UI and links the binary

`make build` includes a guard that fails if `gohookbridge/web/static/index.html` is missing after the web build, so a silent `go build` with an empty embed is impossible.
...
- **Peer discovery** (`raft_discovery.go`) — `NewPeerResolver` selects a
...
- **Bind/advertise separation** (`raft_streamlayer.go`) — bind on
...
- **mTLS** (`raft_tls.go`) — the internal CA is minted by ordinal 0 via
