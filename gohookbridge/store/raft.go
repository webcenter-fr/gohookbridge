...
// bootstrapConfiguration builds the initial Raft cluster configuration from
// the local node plus the configured peers. Peers that reference the local
// node ID are skipped: a peer list that contains every node (as generated by
// the Helm chart, where all pods share the same args) would otherwise produce
// a duplicate-ID configuration, which hashicorp/raft rejects with
// "found duplicate ID in configuration".
func bootstrapConfiguration(cfg RaftConfig) raft.Configuration {
...
func (rs *RaftStore) bootstrap(cfg RaftConfig) error {
	...
	if !hasData {
		configuration := bootstrapConfiguration(cfg)
